...
export class WebhookControlService {
    ...
    public async toggleWebhook({
        ...
        try {
            ...
            if (!instances?.length) {
                this.logger.warn(`No se encontraron instancias activas para userId=${userId}`);
            ...
            this.logger.log(`Enviando solicitud webhook a ${evoAPI} para userId=${userId}`);
            ...
            if (!response.ok) {
                ...
                this.logger.error(`Error en respuesta del servidor Evolution: ${errorDetail}`);
                throw new Error(`Error del servidor Evolution: ${errorDetail}`);
            }

            this.logger.log(`Webhook ${enable ? 'activado' : 'desactivado'} correctamente para userId=${userId}`);
