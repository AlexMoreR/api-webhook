...
export class InstancesService {
  ...
  public async getUserId(instanceName: string) {
    ...
    try {
      ...
      if (!result) {
        this.logger.warn(
          `No se encontró ninguna instancia con el nombre: ${instanceName}`,
        );
      }
      ...
      this.logger.error(
        `Error en getUserId(${instanceName})`,
        error?.message || error,
      );
      throw error;
    }
  ...
  public async getInstances(userId: string) {
    ...
    try {
      ...
      if (!instances.length) {
        this.logger.warn(
          `No se encontraron instancias para el usuario: ${userId}`,
        );
      }
      ...
      this.logger.log(
        `Instancias encontradas para userId=${userId}: ${enrichedInstances.length}`,
      );

      return enrichedInstances;
    } catch (error) {
      this.logger.error(
        `Error en getInstances(userId=${userId})`,
        error?.message || error,
      );
      return [];
    }
