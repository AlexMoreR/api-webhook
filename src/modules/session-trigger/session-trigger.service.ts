...
export class SessionTriggerService {
  ...
  private toSessionIdNumber(sessionId: string | number): number {
    const value = typeof sessionId === 'number' ? sessionId : Number(sessionId);

    if (!Number.isInteger(value) || value <= 0) {
      this.logger.warn(`toSessionIdNumber: sessionId inválido (${sessionId})`);
      throw new BadRequestException(
        'El sessionId debe ser un número entero válido.',
      );
    }
  ...
  async create(
    ...
    this.logger.log(
      `Trigger creado: sessionId=${sessionIdNumber}, time=${time}`,
    );
    return trigger;
  }
  ...
  async updateTimeBySessionId(
    ...
    if (!existing) {
      this.logger.warn(
        `updateTimeBySessionId: No existe trigger con sessionId=${sessionIdNumber}`,
      );
      throw new NotFoundException(
        'No se puede actualizar: trigger no encontrado',
      );
    }
    ...
    this.logger.log(
      `updateTimeBySessionId: time actualizado para sessionId=${sessionIdNumber}`,
    );
    return updated;
  }
  ...
  async findBySessionId(
    ...
    this.logger.log(
      `findBySessionId: Trigger encontrado para sessionId=${sessionIdNumber}`,
    );
    return trigger;
  }
  ...
  async delete(id: number): Promise<SessionTrigger> {
    const existing = await this.prisma.sessionTrigger.findUnique({
      where: { id },
    });
