...
export class LeadClassifierIaService {
  ...
  private async getClientForUser(userId: string): Promise<BaseChatModel> {
    ...
    try {
      ...
      this.logger.debug(
        `[getClientForUser] user defaults=${JSON.stringify(user)}`,
      );
    } catch (err: any) {
      const msg = err?.message || String(err);
      this.logger.error(
        `[getClientForUser] prisma.user.findUnique error: ${msg}`,
        err?.stack || err,
      );
      throw err;
    }
    ...
    try {
      ...
      this.logger.debug(
        `[getClientForUser] userAiConfig found=${!!cfg?.apiKey}`,
      );
    } catch (err: any) {
      const msg = err?.message || String(err);
      this.logger.error(
        `[getClientForUser] prisma.userAiConfig.findFirst error: ${msg}`,
        err?.stack || err,
      );
      throw err;
    }
    ...
    try {
      ...
      this.logger.debug(
        `[getClientForUser] provider=${provider?.name ?? 'null'}`,
      );
    } catch (err: any) {
      const msg = err?.message || String(err);
      this.logger.error(
        `[getClientForUser] prisma.aiProvider.findUnique error: ${msg}`,
        err?.stack || err,
      );
      throw err;
    }
    ...
    try {
      ...
      const msg = err?.message || String(err);
      this.logger.error(
        `[getClientForUser] prisma.aiModel.findUnique error: ${msg}`,
        err?.stack || err,
      );
      throw err;
    }
    ...
    try {
      ...
      this.logger.debug(
        `[getClientForUser] client ready provider=${provider.name} model=${model.name}`,
      );
      return client;
    } catch (err: any) {
      const msg = err?.message || String(err);
      this.logger.error(
        `[getClientForUser] llmClientFactory.getClient error: ${msg}`,
        err?.stack || err,
      );
      throw err;
    }
  ...
  async classify(input: ClassifyMessageDto): Promise<ClassificationResultDto> {
    ...
    try {
      ...
      const msg = err?.message || String(err);
      this.logger.error(
        `[classify] getClientForUser error: ${msg}`,
        err?.stack || err,
      );
    ...
    if (
      !parsed.kind ||
      (parsed.kind !== 'REPORTE' && parsed.kind !== 'REGISTRO')
    ) {
      this.logger.debug(
        `[classify] invalid kind="${parsed.kind}" -> REPORTE fallback`,
      );
      return {
        kind: 'REPORTE',
        sintesis: text.substring(0, 200),
    ...
    this.logger.debug(
      `[classify] parsedOK kind=${parsed.kind} tipo=${parsed.tipo ?? '-'} estado=${parsed.estado ?? '-'}`,
    );
