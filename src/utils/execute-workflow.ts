...
type NodeSenderServiceLike = {
    sendTextNode: (apiUrl: string, apikey: string, remoteJid: string, message: string) => Promise<any>;
};

type ChatHistoryServiceLike = {
    saveMessage: (sessionId: string, message: string, role: 'human' | 'ia') => Promise<any>;
};
...
export async function executeWorkflow(params: {
    ...
    if (!workflowName || workflowName.trim() === '') {
        logger?.warn?.('executeWorkflow: workflowName vacío. No se ejecuta.', context);
        return;
    }
    ...
    if (muteAgentResponses) {
        logger?.log?.(`Agente muteado: workflow ejecutado (${workflowName}) sin mensaje post.`, context);
        return;
    }
...
No menciones "workflow", no repitas el contenido del flujo, no ejecutes acciones ni herramientas.`;
    ...
    const postFlowPrompt = postPromptBuilder ? postPromptBuilder(workflowName) : defaultPrompt;
    ...
    const finalApiMsgUrl = apiMsgUrl ?? `${server_url}/message/sendText/${instanceName}`;
    ...
    for (const [index, block] of blocks.entries()) {
        logger?.log?.(`📤 Enviando bloque post-workflow ${index + 1}/${blocks.length}`, context);
        await nodeSenderService.sendTextNode(finalApiMsgUrl, apikey, remoteJid, block);
